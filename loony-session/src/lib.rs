...
use loony::http::{Payload, RequestHead};
use loony::util::Extensions;
use loony::web::{WebRequest, WebResponse};
use loony::web::{Error, FromRequest, HttpRequest};
...
/// use loony_session::Session;
///
/// fn index(session: Session) -> Result<&'static str, loony::web::Error> {
///     // access session data
///     if let Some(count) = session.get::<i32>("counter")? {
///         session.set("counter", count + 1)?;
///     } else {
...
#[cfg(test)]
mod tests {
    use loony::web::{test, HttpResponse};
